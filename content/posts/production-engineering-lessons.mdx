...
date: "2026-01-20"
excerpt: "Production engineering isn't just about knowing commands — it's about developing the instincts to diagnose systems under pressure, when every second counts and the path forward isn't obvious."
...
This isn't meant to be an exhaustive reference — think of it more as a field guide compiled from real battles in the trenches of production systems.
...
su - appuser            # switch to another user account
```
...
That last command often reveals the primary application — if you see Java listening on port 8080 or Python on 5000, you've found your app.
...
Next, I run `vmstat` with a one-second interval to watch system-wide CPU utilization:
...
When you need to see all active connections, `ss` has replaced the older `netstat`:
...
A classic scenario: you get an error about `/var` being full. Run `df` to confirm, then use `du` to drill down through subdirectories and find the culprit — often it's log files that weren't rotated properly.
...
Production engineering is as much art as science. These commands are tools, but knowing when to use them comes from experience. The key is developing a methodology: start broad, narrow down based on what you find, and always verify your hypotheses.
